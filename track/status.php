...
function render_table() {
  ...
  echo('<p><button id="newcat"> New Status </button></p>');
...
<legend>&nbsp; ' . (($id <= 0) ? 'New' : 'Edit') . ' Status &nbsp;</legend>
...
<table cellpadding="4">
  ...
  <tr>
    ...
    <td><input id="name" name="name" type="text" size="40" value="'.$rec['name'].'" placeholder="Status name." title="Status name."/></td>
  ...
  <tr>
    ...
    <td><input id="notes" name="notes" type="text" size="40" value="'.$rec['notes'].'" placeholder="Status description." title="Status description."/></td>
...
function edit_rec() {
  ...
  if (count($r) > 0) {
    ...
    echo('<br/>Unknown status reference.<br/>');
...
$(function() {
  ...
  $('.xdel').click(function() {
      ...
      if (canBeDeleted(id)) {
        ...
        document.location.replace("status.php?op=del&id=" + id);
  ...
  $('#newcat').click(function () {
      ...
      document.location.replace('status.php?op=new');
...
function canBeDeleted(id) {
  ...
  $.ajax({
    ...
    url: "status.php?op=candel&id=" + id,
    ...
    success: function(json) {
      ...
      if (res) {
        ...
        res = confirm('Are you sure you wish to delete this status record ?');
