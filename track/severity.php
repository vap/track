...
if (($op == 'store') || ($op == 'del') || ($op == 'candel')) {
  ...
  if ($op == 'store') {
    ...
    $res = is_deletable_js('severity', $id);
...
function render_table() {
  ...
  echo('<p><button id="newcat"> New Severity </button></p>');

  $cats = list_severities();

  if (count($cats) == 0) {

    echo('No severities defined yet.');

  } else {

    $hs = array('#', 'Severity', 'Notes');
...
<legend>&nbsp; ' . (($id <= 0) ? 'New' : 'Edit') . ' Severity &nbsp;</legend>
...
<table cellpadding="4">
  ...
  <tr>
    ...
    <td><input id="name" name="name" type="text" size="40" value="'.$rec['name'].'" placeholder="Severity name." title="Severity name."/></td>
  ...
  <tr>
    ...
    <td><input id="notes" name="notes" type="text" size="40" value="'.$rec['notes'].'" placeholder="Severity description." title="Severity description."/></td>
...
function edit_rec() {
  ...
  if (count($r) > 0) {
    ...
    echo('<br/>Unknown severity reference.<br/>');
...
function severity() {
  ...
  switch ($op) {
    ...
    /* -- This condition has been promoted to the top of file.
...
$(function() {
  ...
  $('.xdel').click(function() {
      ...
      if (canBeDeleted(id)) {
        ...
        document.location.replace("severity.php?op=del&id=" + id);
  ...
  $('#newcat').click(function () {
      ...
      document.location.replace('severity.php?op=new');

    });

  $('#cancel').click(function () {

      document.location.replace('severity.php');
...
function canBeDeleted(id) {
  ...
  $.ajax({
    ...
    url: "severity.php?op=candel&id=" + id,
    ...
    success: function(json) {
      ...
      if (res) {
        ...
        res = confirm('Are you sure you wish to delete this severity record?');
