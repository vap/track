...
if (($op == 'store') || ($op == 'del') || ($op == 'candel')) {
  ...
  if ($op == 'store') {
    ...
    do_del('primetrack', $id);

  } else {

    $res = is_deletable_js('primetrack', $id);
  ...
  header('Location: primetrack.php');
...
function render_table() {
  ...
  echo('<p><button id="newcat"> New Primary Track </button></p>');



  $cats = list_accessible_primetracks($_SESSION['user']['id']); // list_primetracks();

  if (count($cats) == 0) {

    echo('No primary track defined yet. Or, there are no primary tracks that you have access to.');

  } else {

    $hs = array('#', 'Primary Track', 'Notes');
...
<legend>&nbsp; ' . (($id <= 0) ? 'New' : 'Edit') . ' Primary Track &nbsp;</legend>
...
<table cellpadding="4">
  ...
  <tr>
    ...
    <td><input id="name" name="name" type="text" size="40" value="'.$rec['name'].'" placeholder="Primary Track name." title="Primary Track name."/></td>
  ...
  <tr>
    ...
    <td><input id="notes" name="notes" type="text" size="40" value="'.$rec['notes'].'" placeholder="Primary Track description." title="Primary Track description."/></td>
...
function edit_rec() {
  ...
  if (count($r) > 0) {
    ...
    echo('<br/>Unknown primary track reference.<br/>');
...
$(function() {
  ...
  $('.xdel').click(function() {
      ...
      if (canBeDeleted(id)) {
        ...
        document.location.replace("primetrack.php?op=del&id=" + id);
  ...
  $('#newcat').click(function () {
      ...
      document.location.replace('primetrack.php?op=new');

    });

  $('#cancel').click(function () {

      document.location.replace('primetrack.php');
...
function canBeDeleted(id) {
  ...
  $.ajax({
    ...
    url: "primetrack.php?op=candel&id=" + id,
    ...
    success: function(json) {
      ...
      if (res) {
        ...
        res = confirm('Are you sure you wish to delete this primary track record ?');
